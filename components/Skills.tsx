...
function Skills({skills}: Props) {
  return (
    <motion.main 
        ...
        className='h-screen flex relative flex-col text-center md:text-left ml:flex-row max-w-[2088px] xl:px-10 min-h-screen justify-center  mx-auto items-center'
    >
        ...
        <div className='mt-12 grid grid-cols-4 gap-5 md:mt-40'>
            {skills?.map((skill)=>{
                return (
                    <main key={skill?._id} className='group relative flex cursor-pointer justify-center items-center'>
                        <motion.section
                            initial={{
                                x: 200,
                                opacity: 0,
